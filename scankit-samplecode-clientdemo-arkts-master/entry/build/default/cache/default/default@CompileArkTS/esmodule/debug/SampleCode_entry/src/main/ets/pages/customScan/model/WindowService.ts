...
const WINDOW_SIZE_MOCK: window.Size = {
    width: 1080,
    height: 1920
};
@Observed
    ...
    public setTopAvoidHeight(): void {
        if (this.windowObj) {
            try {
                ...
                Logger.info(TAG, `Succeeded in getting topAvoidHeight by windowObj. topAvoidHeight: ${this.topAvoidHeight}`);
            }
            catch (error) {
    ...
    public setWindowSize(): void {
        if (this.windowObj) {
            try {
                ...
                Logger.info(TAG, `Succeeded in getting windowRect by windowObj. width: ${windowRect.width}, height: ${windowRect.height}`);
                this.emitWindowSize({
                ...
                this.emitWindowSize(WINDOW_SIZE_MOCK);
            }
        }
    }

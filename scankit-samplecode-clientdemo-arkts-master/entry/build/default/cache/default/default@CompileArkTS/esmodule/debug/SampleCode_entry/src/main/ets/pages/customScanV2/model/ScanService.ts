...
export const TAG = 'ScanServiceV2';
@ObservedV2
...
export enum ScanStatus {
    ...
    PHOTO_DECODING = "PHOTO_DECODING",
    NO_PHOTO_SELECT = "NO_PHOTO_SELECT",
    PREVIEW_DECODING_COMPLETED = "PREVIEW_DECODING_COMPLETED",
    PHOTO_DECODING_COMPLETED = "PHOTO_DECODING_COMPLETED" // Gallery decoding finished.
}
@ObservedV2
export class ScanServiceV2 {
    @Trace
    ...
    private static instance: ScanServiceV2 | null = null;
    public static getInstance(): ScanServiceV2 {
        if (ScanServiceV2.instance === null) {
            ScanServiceV2.instance = new ScanServiceV2();
        }
        return ScanServiceV2.instance;
    }
    ...
    public startPreviewStream(options: scanBarcode.ScanOptions, viewControl: customScan.ViewControl, callback: AsyncCallback<Array<scanBarcode.ScanResult>>) {
        Logger.info(TAG, `Start Preview Stream. Width: ${viewControl.width}, height: ${viewControl.height}, scanStatus: ${this.scanStatus}`);
    ...
    public retryOnCondition(error: BusinessError, options: scanBarcode.ScanOptions, viewControl: customScan.ViewControl, callback: AsyncCallback<Array<scanBarcode.ScanResult>>): boolean {
        if (error) {
            Logger.error(TAG, `An error is returned during customScanCallback. Code: ${error.code}, message: ${error.message}`);
    ...
    private startCustomScan(viewControl: customScan.ViewControl, callback: AsyncCallback<Array<scanBarcode.ScanResult>>): void {
        try {
    ...
    private onLightingFlash() {
        try {
            customScan.on('lightingFlash', (error, isLightingFlash) => {
                if (error) {
                    Logger.error(TAG, `lightingFlash callback error. Code: ${error.code}, message: ${error?.message}`);
                    return;
                }
            ...
            Logger.error(TAG, `lightingFlash callback error by catching. Code: ${error?.code}, message: ${error?.message}`);
        }
    }
